...
import freestyle.free.rpc.protocol._
  ...
  val serviceBindings: Defn.Def = {
    ...
    q"""
       def bindService[F[_]](implicit algebra: $algName[F], EFF: _root_.cats.effect.Effect[F], HTask: _root_.freestyle.free.FSHandler[F, _root_.monix.eval.Task], C: _root_.cats.Comonad[F], S: _root_.monix.execution.Scheduler): _root_.io.grpc.ServerServiceDefinition =
           new _root_.freestyle.free.rpc.internal.service.GRPCServiceDefBuilder(${Lit.String(
...
private[internal] case class RPCRequest(
  ...
  val encodersImport: Import = serialization match {
    case Protobuf =>
      q"import _root_.pbdirect._, _root_.freestyle.free.rpc.internal.service.encoders.pbd._"
    case Avro =>
      q"import _root_.freestyle.free.rpc.internal.service.encoders.avro._"
  }
    ...
    case Some(RequestStreaming) =>
      q"""
         def $name(input: _root_.monix.reactive.Observable[$requestType]): M[$responseType] =
           _root_.freestyle.free.rpc.internal.client.calls.clientStreaming(input, $descriptorName, channel, options)
       """
    case Some(ResponseStreaming) =>
      q"""
         def $name(request: $requestType): _root_.monix.reactive.Observable[$responseType] =
           _root_.freestyle.free.rpc.internal.client.calls.serverStreaming(request, $descriptorName, channel, options)
       """
    case Some(BidirectionalStreaming) =>
      q"""
         def $name(input: _root_.monix.reactive.Observable[$requestType]): _root_.monix.reactive.Observable[$responseType] =
           _root_.freestyle.free.rpc.internal.client.calls.bidiStreaming(input, $descriptorName, channel, options)
       """
    case None =>
      q"""
         def $name(request: $requestType): M[$responseType] =
           _root_.freestyle.free.rpc.internal.client.calls.unary(request, $descriptorName, channel, options)
      """
  }
  ...
  val call: Term.Tuple = {
    streamingType match {
      case Some(RequestStreaming) =>
        q"""
         ($descriptorName,
         _root_.io.grpc.stub.ServerCalls.asyncClientStreamingCall(_root_.freestyle.free.rpc.internal.service.calls.clientStreamingMethod(algebra.$name)))
       """
      case Some(ResponseStreaming) =>
        q"""
         ($descriptorName,
         _root_.io.grpc.stub.ServerCalls.asyncServerStreamingCall(_root_.freestyle.free.rpc.internal.service.calls.serverStreamingMethod(algebra.$name)))
       """
      case Some(BidirectionalStreaming) =>
        q"""
         ($descriptorName,
         _root_.io.grpc.stub.ServerCalls.asyncBidiStreamingCall(_root_.freestyle.free.rpc.internal.service.calls.bidiStreamingMethod(algebra.$name)))
       """
      case None =>
        q"""
          ($descriptorName,
         _root_.io.grpc.stub.ServerCalls.asyncUnaryCall(_root_.freestyle.free.rpc.internal.service.calls.unaryMethod(algebra.$name)))
       """
    }
  }
}
