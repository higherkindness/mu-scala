...
* **`srcGenSourceFromJarsDir`**: The IDL from jars directory, where your IDL definitions extracted from jars are placed. By default is `(Compile / resourceManaged) / idlType`, typically `target/scala-2.12/src_managed/main/avro`
...
* **`srcGenSourceDirs`**: the sequence of IDL source base directories, where your IDL files are placed. By default contains `srcGenSourceDir` and `srcGenSourceFromJarsDir`. In the jar, it will maintain the packages structure you define.
