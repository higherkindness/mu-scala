...
final class HydrationMacros(val c: whitebox.Context) {
  ...
  def hydrate[D <: Dehydrated, F[_]](alg: c.Expr[Algebra[F]])(
    ...
    (
      for {
        ...
        _ = scala.Predef.println(s"Today's hydration brought to you by ${hydration.getClass}")
      } yield hydration.hydrate(FSym, nodeInfos)
    ) fold (error => c.abort(c.enclosingPosition, error),
  ...
  @tailrec
  final def hlistTypeFoldLeft[A](tpe: Type)(a0: A)(f: (A, Type) => A): Either[String, A] =
    tpe match {
