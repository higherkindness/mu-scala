...
//  val MonixServicePrefix = "monix"
  ...
  val unaryRoute: HttpRoutes[IO] = UnaryGreeter.route[IO]

  val server: BlazeServerBuilder[IO] = BlazeServerBuilder[IO]
    .bindHttp(Port, Hostname)
    .withHttpApp(
      Router(
        s"/$UnaryServicePrefix" -> unaryRoute,
//        s"/$Fs2ServicePrefix"   -> fs2Service,
//        s"/$MonixServicePrefix" -> monixService
      ).orNotFound)
...
//  val fs2ServiceClient: Fs2GreeterRestClient[IO] = new Fs2GreeterRestClient[IO](serviceUri / Fs2ServicePrefix)
//  val monixServiceClient: MonixGreeterRestClient[IO] = new MonixGreeterRestClient[IO](serviceUri / MonixServicePrefix)
